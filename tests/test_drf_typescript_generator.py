...
from drf_typescript_generator.utils import (
    _get_method_return_value_type,
    _get_typescript_name,
...
from tests.utils import (
    ...
    ListFieldTestSerializer,
    MethodOutputTestSerializer,
    ModelTestSerializer,
    NestedSerializersTestSerializer,
    TypescriptNameTestSerializer,
)
...
class TestRequired(BaseTest):
    ...
    def test_typescript_not_required_name(self):
        ts_name = _get_typescript_name(
            self.fields["not_required_field"], "not_required_field"
        )
...
class TestChoiceField(BaseTest):
    ...
    def test_basic_choice_selection_fields(self):
        ts_type = _get_typescript_type(
            self.fields["choice_field_int"], "choice_field_int", self.serializer
        )
        assert ts_type == "1 | 2 | 3"
        ts_type = _get_typescript_type(
            self.fields["choice_field_float"], "choice_field_float", self.serializer
        )
        assert ts_type == "1.2 | 3.1"
        ts_type = _get_typescript_type(
            self.fields["choice_field_bool"], "choice_field_bool", self.serializer
        )
        assert ts_type == "true | false"
        ts_type = _get_typescript_type(
            self.fields["choice_field_str"], "choice_field_str", self.serializer
        )
        assert ts_type == '"a" | "b"'

    def test_choice_selection_fields_with_empty_values(self):
        ts_type = _get_typescript_type(
            self.fields["choice_field_str_blank"],
            "choice_field_str_blank",
            self.serializer,
        )
        assert ts_type == '"a" | "b" | ""'
        ts_type = _get_typescript_type(
            self.fields["choice_field_int_null"],
            "choice_field_int_null",
            self.serializer,
        )
...
class TestListField(BaseTest):
    ...
    def test_composite_list_field_type(self):
        ts_type = _get_typescript_type(
            self.fields["composite_lst"], "composite_lst", self.serializer
        )
        assert ts_type == "(number | null)[]"

    def test_multiple_choice_field(self):
        ts_type = _get_typescript_type(
            self.fields["multichoice"], "multichoice", self.serializer
        )
        assert ts_type == "(1 | 2 | 3)[]"
...
def test_model_serializer():
    ...
    assert (
        " ".join(ts_serializer.split()).strip()
        == " ".join(
            """
...
class TestNestedSerializers(BaseTest):
    ...
    def test_nullable_many_objects_nested_serializer(self):
        ts_type = _get_typescript_type(
            self.fields["models_nullable"], "models_nullable", self.serializer
        )
